...
class ActivationBuffer:
    ...
    def __init__(self, 
        ...
        self.device = device
        self.sequential = sequential  # 保存顺序采样标志
        
        # 顺序采样的额外状态
        if self.sequential:
            self.sequential_index = 0  # 当前顺序索引
    
    def __iter__(self):
        if self.sequential:
            self.sequential_index = 0  # 重置顺序索引
        return self
    ...
    def __next__(self):
        ...
        with t.no_grad():
            ...
            if self.sequential:
                # 顺序采样
                if self.sequential_index >= len(unreads):
                    raise StopIteration("All activations have been read sequentially")
                
                # 取连续的一批
                end_idx = min(self.sequential_index + self.out_batch_size, len(unreads))
                idxs = unreads[self.sequential_index:end_idx]
                self.sequential_index = end_idx
            else:
                # 原来的随机采样
    ...
    def config(self):
        ...
        if hasattr(self, 'sequential'):
            config['sequential'] = self.sequential
        return config
