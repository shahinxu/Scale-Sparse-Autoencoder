...
class ScaleNoiseAutoEncoder(Dictionary, nn.Module):
    """
    ...
    def encode(self, x):
        ...
        if self.training:
            noise = t.randn_like(z) * 0.01  # You can adjust the noise scale
            z = z + noise
    ...
    def decompose_low_high(self, M, scale: t.Tensor):
        ...
        A_LP_base = self.lp_transform(expert_avg)
    ...
    def from_pretrained(path, k=100, experts=16, e=1, heaviside=False, device=None):
        ...
        autoencoder = ScaleNoiseAutoEncoder(activation_dim, dict_size, k, experts, e, heaviside)
