...
from dictionary_learning.trainers.moe_physically_simple_scale import MultiExpertSimpleScaleAutoEncoder, MoETrainer
...
with open("metrics_log.jsonl", "a") as f:
    for i, trainer_config in enumerate(trainer_configs):
        ae = MultiExpertSimpleScaleAutoEncoder.from_pretrained(
...
#         ae.to(device)
#         metrics = evaluate(ae, buffer, device=device, n_batches=100)
#         safe_config = {k: (str(v) if callable(v) or isinstance(v, type) else v) for k, v in trainer_config.items()}
#         record = {"trainer_config": safe_config, "metrics": metrics}
#         f.write(json.dumps(record, ensure_ascii=False, default=str) + "\n")
#         print(record)